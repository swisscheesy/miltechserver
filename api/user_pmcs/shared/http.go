...
func IfNoneMatchMatches(fieldValues []string, currentETag string) (bool, *APIError) {
	...
	if !valid {
		return false, NewInternalError("invalid current ETag", nil)
	}
...
func invalidIfNoneMatch() *APIError {
	return NewInvalidPrecondition("If-None-Match must be * or a valid entity-tag list", nil)
}
