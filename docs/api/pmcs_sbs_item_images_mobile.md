...
|-----------|------|----------|-------|
| `blob_path` | string | Yes | Full PMCS SBS guide JSON blob path. Use the selected guide's `blob_path` value from `GET /library/pmcs-sbs/:folder/files`. |
...
| Field | Source |
|-------|--------|
| `blob_path` | Selected guide JSON blob path. |
...
| Invalid `blob_path` prefix, separators, or path traversal | `400` | `{"error":"Invalid request","details":"invalid blob path: must start with pmcs_sbs/"}` |
...
`GET /api/v1/library/pmcs-sbs/image?blob_path=pmcs/other/file.json&image_name=Before_12`
...
`GET /api/v1/library/pmcs-sbs/image?blob_path=pmcs_sbs/HMMWV/HMMWV%20NoArmor%20(SEPT13).json&image_name=Before_12.png`
...
`GET /api/v1/library/pmcs-sbs/image?blob_path=pmcs_sbs/HMMWV/HMMWV%20NoArmor%20(SEPT13).json&image_name=Missing_99`
...
- Use the guide `blob_path` returned by the PMCS SBS List Files endpoint as `blob_path`.
...
- Cache image bytes by `blob_path` plus `image_name` if local image caching is needed.
